...
public class WorldGenerator {
    ...
    //记录房间的位置
    private static TETile[][] positionOfRoom;//我把这个暂时拿掉了，目前还是可以造房间的
    ...
    public TETile[][] mapGenerator(){
        ...
        do{
            ...
            fillWithRoom();
            //在房间外打通出迷宫
        }while (!HallWay.generateHallWay(world,SEED));
        //在房间上开口
...
//            }
    ...
    private void initializeWorldArray(){
        for(int i=0;i<WIDTH;i++){
            for(int j=0;j<HEIGHT;j++){
                positionOfRoom[i][j]= Tileset.NOTHING;
    ...
    private boolean DFS(int x, int y){
        ...
        for(int i=0; i<=3;i++){
            ...
            if(Tileset.FLOOR.equals(world[mx][my])){
                accessiblePath.add(new Position(mx,my));  //offer
            }
        }
    ...
    public static void main(String[] args) {
        ...
        WorldGenerator world=new WorldGenerator(100,60,230098098);
